...
export default function EventTable(props) {
  ...
  const handleReserveButtonClick = (event) => {
    ...
    reserveEvent(event.id).then((response) => {
  ...
  const cell = (event) => (
    <View style={styles.cell}>
      <Text style={event.status === 'FREE' ? styles.text : styles.grayed}>
        {event.start.split('T')[1].substr(0, 5) +
          ' - ' +
          event.end.split('T')[1].substr(0, 5)}
      </Text>
      <Text style={event.status === 'FREE' ? styles.text : styles.grayed}>
        Status: {event.status}
      </Text>
            ...
            <Table borderStyle={{ borderWidth: 1, borderColor: 'black' }}>
              <TableWrapper style={{ flex: 1 }}>
                <Cols
                  data={tableData}
                  widthArr={widthArr}
                  heightArr={[100, 100, 100, 100, 100, 100, 100, 100]}
